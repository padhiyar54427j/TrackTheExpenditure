...
table_name = "JIGAR"
conn = sqlite3.connect(r"TrackTheExpenditure.db")
cur = conn.cursor()
...
def name():
    ...
    try:
        ...
        cur.execute("CREATE TABLE " + str(
            table_name) + " (Date_Of_Payment text,Method_Of_Payment text, Paid_To text,Description text,Amount_Paid REAL)")
        messagebox.showinfo("SUCCESS", "User " + str(table_name) + " has been created")
        _entry = StringVar()
        _entry.set("Current User(in use): " + table_name)
        ...
        screen.title("Current Total Expenditure : Rupees " + str(tot))
        conn.commit()
        conn.close()
        new_screen2.destroy()
        win.destroy()
        ...
        messagebox.showwarning("WARNING", "PLEASE ENTER A VALID USER NAME")
...
def create_table():
    ...
    new_screen2.iconbitmap(r'icons8-money-box-50.ico')
    new_screen2.title("Create New User")
    Label(new_screen2, text="Please enter the name of the new User").pack()
...
def main_screen():
    ...
    for amount in amt:
        tot += int(amount[0])
    screen = Tk()
    screen.iconbitmap(r'icons8-money-box-50.ico')
    screen.geometry("700x500+150+135")
    screen.title("Current Total Expenditure : Rupees " + str(tot))
    conn.commit()
    conn.close()
    my_img = ImageTk.PhotoImage(Image.open(r"TrackTheExpenditure.png"))
    button1_img = ImageTk.PhotoImage(Image.open(r"button1.png"))
    button_img = ImageTk.PhotoImage(Image.open(r"button.png"))
    bg_img = ImageTk.PhotoImage(Image.open(r"bg2_.png"))
    load = Image.open(r"TrackTheExpenditure.png")
    image = load.resize((700, 80), Image.ANTIALIAS)
    render = ImageTk.PhotoImage(image)

    load3 = Image.open(r"button.png")
    image3 = load3.resize((276, 50), Image.ANTIALIAS)
    render3 = ImageTk.PhotoImage(image3)

    load1 = Image.open(r"button1.png")
    image1 = load1.resize((276, 50), Image.ANTIALIAS)
    render1 = ImageTk.PhotoImage(image1)

    load2 = Image.open("exit.png")
    image2 = load2.resize((100, 30), Image.ANTIALIAS)
    render2 = ImageTk.PhotoImage(image2)

    load4 = Image.open(r"create_new_table.png")
    image4 = load4.resize((176, 50), Image.ANTIALIAS)
    render4 = ImageTk.PhotoImage(image4)

    load5 = Image.open(r"update.png")
    image5 = load5.resize((176, 50), Image.ANTIALIAS)
    render5 = ImageTk.PhotoImage(image5)

    load6 = Image.open(r"manage.png")
...
def manage():
    ...
    tables()
    win = Toplevel(screen)
    win.iconbitmap(r'icons8-money-box-50.ico')
    win.title("MANAGE USERS")
    Button(win, image=render4, command=create_table, relief=SOLID, bd=1).pack(pady=30, expand=True)
    Button(win, text="DELETE SELECTED USER", font=("BOLD", 10), bg="orange", command=remove_table, relief=SUNKEN,
           bd=1).pack(padx=30, ipadx=30, pady=10, ipady=10)

    Label(win, text="Choose the User that you want to use:", font=("Bold", 15)).pack()
    lt = []
    lst = []
    n = 0
...
def remove_table():
    ...
    for _i in range(0, len(lst)):
        ...
        if (lt[_i].get()) == 1:
            table_name = lst[_n]
            if table_name == "JIGAR":
                messagebox.showwarning("Warning", "Can't delete the default table 'JIGAR'")

            else:
                cur.execute("DROP TABLE " + table_name)
                messagebox.showinfo("SUCCESS", "Successfully delete User " + table_name)
                table_name="JIGAR"
...
def change():
    ...
    for _i in range(0, len(lst)):
        ...
        if (lt[_i].get()) == 1:
            table_name = lst[_n]
            messagebox.showinfo("SUCCESS", "The current User has been changed to- '" + table_name + "'")
            entry = StringVar()
            _entry.set("Current User(in use): " + table_name)
        ...
        _n += 1
    if tempp == 0:
        messagebox.showwarning("WARNING", "SELECT ATLEAST ONE USER TO WORK ON")
        win.destroy()
        manage()
    else:
        win.destroy()
...
def new_Records():
    ...
    new_screen.iconbitmap(r'icons8-money-box-50.ico')
...
def previous_Records():
    ...
    new_screen1.iconbitmap(r'icons8-money-box-50.ico')
    ...
    Button(new_screen1, text="DELETE THE SELECTED RECORD", bg="orange", font=("BOLD", 15), command=remove).pack(padx=10,
                                                                                                             pady=10,
                                                                                                             ipadx=220)
    ...
    Button(new_screen1, text="UPDATE RECORD", bg="lightgreen", font=("BOLD", 15), command=update1).pack(padx=25, pady=10,
                                                                                                 ipadx=300)
